...
namespace Die
{
    public class DieLandingValidator
    {
        ...
        public DieValue Validate(Transform transform, float accumulatedDistance, float accumulatedAngularMovement)
        {
            if (accumulatedDistance < MIN_DISTANCE || accumulatedAngularMovement < MIN_ANGULAR_VELOCITY)
                return null;
            
            var bestMatch = _valuesHolder.DiceFaces
                .Select(face => new {face, alignment = Vector3.Dot(transform.TransformDirection(face.Normal), Vector3.up)})
